...
function ProductDetails() {
  ...
  const currentVariant = useMemo(() => {
    if (product?.hasVariants) {
      ...
      ); // fallback to first if none strictly match yet
    }
    return null;
    ...
    <div className="product-details container section">
        ...
        <img
          src={
            selectedImage?.startsWith("http") || selectedImage?.startsWith("//")
              ? selectedImage
              : `//${window.location.host}/api/images/${selectedImage}`
          }
          alt={product.name}
              ...
              <img
                ...
                src={
                  img?.startsWith("http") || img?.startsWith("//")
                    ? img
                    : `//${window.location.host}/api/images/${img}`
                }
      ...
      <div className="details-content">
        ...
        <div className="price-section">
          {(() => {
            const displayPrice = currentVariant
              ...
              : product.price;
            const displayMrp = currentVariant
              ? currentVariant.mrp
              : product.mrp;
        ...
        <p
          ...
          style={{ marginTop: "15px", lineHeight: "1.6", color: "#444" }}
        >
          {product.description}
        </p>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: colorImage
                          ? `url(${colorImage?.startsWith("http") || colorImage?.startsWith("//") ? colorImage : `//${window.location.host}/api/images/${colorImage}`}) center/cover`
                          : selectedColor === color
                            ? "#f0f8ff"
                            : "#fff",
                        cursor: "pointer",
                        ...
                        textShadow: colorImage
                          ? "1px 1px 3px rgba(0,0,0,0.9), 0px 0px 5px rgba(0,0,0,0.6)"
                          : "none",
                        fontWeight: "bold",
                        fontSize: "11px",
                        lineHeight: "1.2",
          ...
          {product.deliveryAvailable !== false && (
            ...
            >
              ...
              {product.freeDeliveryAbove > 0
                ? `Free Delivery Above ₹${product.freeDeliveryAbove}`
                : "Free Delivery"}
          ...
          >
            {(currentVariant?.sku || product.sku) && (
              <li style={{ display: "flex", fontSize: "14px" }}>
                <span style={{ width: "120px", color: "#666" }}>SKU:</span>{" "}
                <strong>{currentVariant?.sku || product.sku}</strong>
              </li>
            )}
            {product.weight > 0 && (
              <li style={{ display: "flex", fontSize: "14px" }}>
                <span style={{ width: "120px", color: "#666" }}>Weight:</span>{" "}
                <strong>
            ...
            {product.manufacturer && (
              <li style={{ display: "flex", fontSize: "14px" }}>
                <span style={{ width: "120px", color: "#666" }}>
                  Manufacturer:
                </span>{" "}
            ...
            {product.countryOfOrigin && (
              <li style={{ display: "flex", fontSize: "14px" }}>
                <span style={{ width: "120px", color: "#666" }}>Origin:</span>{" "}
                <strong>{product.countryOfOrigin}</strong>
              </li>
            )}
            {product.expiryDate && (
              <li style={{ display: "flex", fontSize: "14px" }}>
                <span style={{ width: "120px", color: "#666" }}>Expiry:</span>{" "}
        ...
        >
          {(product.hasVariants && currentVariant
            ? currentVariant.stock
            : product.stock) > 0 || product.allowOutOfStockPurchase ? (
            <>
              <button
        ...
        <div className="review-section" style={{ marginTop: "40px" }}>
          <ReviewForm productId={product._id} onReviewAdded={fetchProduct} />
        </div>
          ...
          >
            {reviews.map((review) => (
              ...
              >
                ...
                <p
                  style={{ margin: "5px 0 0", color: "#555", fontSize: "14px" }}
                >
                  {review.comment}
                </p>
              </div>
